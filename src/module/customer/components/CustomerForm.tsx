...
import {
  CrmSelect,
  OptionType,
} from "../../../utils/components/core/SelectInput.tsx";
...
export const CustomerForm = () => {
  ...
  switch (formType) {
    ...
    case CrmFormType.REGISTER_CUSTOMER:
      return (
        <CrmModal open={true} onClose={() => setFormType(CrmFormType.EMPTY)}>
          <CustomerRegister />
        </CrmModal>
      );
...
const CustomerRegister = ({ customerUUID }: { customerUUID?: string }) => {
    ...
    if (customerUUID) {
      customersUseCase
        .saveCustomer({
          ...
          status: data.status,
          economicActivitiesCodes: data.economicActivitiesCodesForm.map(
        ...
        .then((response) => {
          if (response.error) {
            ...
            popup.toast(
              "success",
              t("customers.messages.customer_included_success"),
              2000
            );
  ...
  const handleGetPreCustomer = async (document: string) => {
    try {
      setLoading(true);

      const response = await customersUseCase.getPreCustomer(document);

      if (response.error) {
        popup.toast("error", t(`pre_customer.errors.${response.error}`), 2000);
      }
      ...
      if (response.customer) {
        ...
        preCustomer.economicActivitiesCodes?.forEach((x, i) => {
          if (i >= economicActivitiesCodes.fields.length) {
            economicActivitiesCodes.append({ value: x });
          } else {
      ...
      <FormProvider {...formMethods}>
        ...
        >
          <Typography level={"body-md"} fontWeight={"bold"}>
            {customerUUID ? t("actions.edit") : t("actions.register")}{" "}
          ...
          <Tabs defaultValue={0} sx={{ pt: 0.5 }}>
            <TabList>
              ...
              <Tab disabled={loading}>
                {t("customers.page.tabs.comercial_informations")}
              </Tab>
            </TabList>
            <TabPanel value={0} sx={{ pl: 0, pr: 0 }}>
              <Box display={"flex"} alignItems={"center"} gap={1}>
                      ...
                      loading ? (
                        ...
                        <IconButton
                          onClick={() => {
                            ...
                            const query =
                              (inputCNPJ.current as HTMLInputElement).value ??
                              "";
                ...
                <FormControl sx={{ flex: 1 }}>
                  ...
                  <TextInput
                    {...register("personName", {
                      required: t("customers.messages.person_name_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
              ...
              <FormControl>
                ...
                <TextInput
                  {...register("companyName", {
                    required: t("customers.messages.company_name_required"),
                  })}
                  size={"sm"}
                  variant={"soft"}
                  disabled={loading}
              ...
              <FormControl>
                ...
                <TextInput
                  {...register("tradingName", {
                    required: t("customers.messages.trading_name_required"),
                  })}
                  size={"sm"}
                  variant={"soft"}
                  disabled={loading}
              ...
              <Box display={"flex"} alignItems={"center"} gap={1}>
                <FormControl>
                  ...
                  <ZipCodeInput
                    {...register("zipCode", {
                      required: t("customers.messages.zip_code_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
                ...
                <FormControl>
                  ...
                  <TextInput
                    {...register("city", {
                      required: t("customers.messages.city_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
                ...
                <FormControl>
                  ...
                  <TextInput
                    {...register("state", {
                      required: t("customers.messages.state_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
                ...
                <FormControl>
                  ...
                  <TextInput
                    {...register("complement", {
                      required: t("customers.messages.complement_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
              ...
              <Box display={"flex"} alignItems={"center"} gap={1}>
                <FormControl sx={{ flex: 1 }}>
                  ...
                  <TextInput
                    {...register("address", {
                      required: t("customers.messages.address_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
                ...
                <FormControl>
                  ...
                  <NumericInput
                    {...register("addressNumber", {
                      required: t("customers.messages.number_required"),
                    })}
                    size={"sm"}
                    variant={"soft"}
                    disabled={loading}
            ...
            <TabPanel value={1} sx={{ pl: 0, pr: 0 }}>
              ...
              >
                ...
                <Divider sx={{ mt: 2 }}>
                  {t("customers.page.sections.economic_activities")}
                </Divider>
                <Box
                  sx={{
                    display: "flex",
                  ...
                  {economicActivitiesCodes.fields.map((_, i) => (
                    ...
                    >
                      <FormControl sx={{ flex: 1 }}>
                        <NumericInput
                          {...register(
                            `economicActivitiesCodesForm.${i}.value`
                          )}
                          size={"sm"}
                          variant={"soft"}
...
const ApprovalCustomer = ({ customerUUID }: { customerUUID: string }) => {
  ...
  const handleFormApproval = handleSubmit((data: FieldValues) => {
    customersUseCase
      .approvalCustomer(
        customerUUID,
        CustomerStatus[data.status as keyof typeof CustomerStatus]
      )
      .then((response) => {
        if (response.error) {
          popup.toast("error", t(`customers.errors.${response.error}`), 2000);
  ...
  const address = () => {
    return `${maskZipCode(customer?.zipCode ?? "")} ${
      customer?.address ?? ""
    }, Nº ${customer?.addressNumber ?? ""} - ${customer?.city ?? ""} ${
      customer?.state ?? ""
    }`;
  };
        ...
        >
          ...
          >
            {activities.map((a, i) => (
              <CustomerEconomicActivity
                activity={a}
                key={`customer_economic_activity_${i}`}
              />
            ))}
          </Box>
          <Box
          ...
          >
            ...
            <Button
              type={"submit"}
              sx={{ flex: 1 }}
              startDecorator={<PublishedWithChangesRounded />}
            >
              {t("actions.save")}
            </Button>
          </Box>
...
export const CustomerEconomicActivity = (props: {
      ...
      <Accordion expanded={open}>
        ...
        <AccordionDetails>
          ...
          >
            ...
            >
              <Typography level={"body-sm"} fontWeight={"bold"}>
                {t("customers.fields.session")}{" "}
                {props.activity.section?.code ?? ""}
              </Typography>
              <Typography
                level={"body-sm"}
                sx={{
            ...
            >
              <Typography level={"body-sm"} fontWeight={"bold"}>
                {t("customers.fields.division")}{" "}
                {props.activity.division?.code ?? ""}
              </Typography>
              <Typography
                level={"body-sm"}
                sx={{
