...
export const WalletsList = () => {
  ...
  const canCreate =
    roles.filter((x) => x.code === "CREATE_WALLET" || x.code === "ALL_WALLET")
      .length > 0;
  ...
  const walletStatusOptions: CrmFieldOption[] = [
  ...
  const walletFieldsFilter = [
    ...
    },
    {
      ...
      filterOptions: walletStatusOptions,
    },
    // { value: "created_at", label: t("wallets.fields.created_at") }, Depois que tiver uma estrutura para isso implementar, tanto no front como campo do tipo data quanto no back
  ];

  const walletFields = [
    ...walletFieldsFilter,
    { value: "created_at", label: t("wallets.fields.created_at") },
      ...
      <CrmTableContainer sx={{ height: 450, pt: 2 }}>
        ...
        >
          <thead>
            <tr>
              <CrmTableHead
                field={walletFields.find((x) => x.value === "label")!}
                orderByAtom={WalletState.OrderByAtom}
              />
              <CrmTableHead
                field={walletFields.find((x) => x.value === "accountable")!}
                orderByAtom={WalletState.OrderByAtom}
              />
              <CrmTableHead
                field={
                  walletFields.find((x) => x.value === "customers_quantity")!
                }
                orderByAtom={WalletState.OrderByAtom}
              />
              <CrmTableHead
                field={walletFields.find((x) => x.value === "status")!}
                orderByAtom={WalletState.OrderByAtom}
              />
              <CrmTableHead
                field={walletFields.find((x) => x.value === "created_at")!}
                orderByAtom={WalletState.OrderByAtom}
              />
              {canCreate && (
                <CrmTableHead
                  field={
                    walletFields.find((x) => x.value === t("actions.edit"))!
                  }
                  orderByAtom={null}
                />
              )}
